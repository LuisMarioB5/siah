...
export function ScheduleTable({
  ...
  // Apply day filter
  if (dayFilter !== "all") {
    const capitalizedDay = dayFilter.charAt(0).toUpperCase() + dayFilter.slice(1)
    const filteredDays = {}
    filteredDays[capitalizedDay] = schedule[capitalizedDay]
    Object.assign(filteredSchedule, filteredDays)
  }

  // For day view, we only show one day
  const displayDays =
    viewType === "day"
      ? dayFilter !== "all"
        ...
        <TableBody>
          {hours.map((hour) => (
            <TableRow key={hour}>
              ...
              {displayDays.map((day) => {
                const cells = filteredSchedule[day][hour]
                ...
                // Filter cells based on course and teacher filters
                const filteredCells = cells.filter((cell) => {
                  if (!cell.isBreak) {
                    const courseMatch = courseFilter === "all" || cell.course === courseFilter
                    const teacherMatch =
                      teacherFilter === "all" || cell.teacher.toLowerCase().includes(teacherFilter.toLowerCase())
